...
if(!encrp.save(filename)) return false;
...
if(!encrp.load(filename)) return false;
...
mac_input.insert(mac_input.end(), mac_key.begin() + 16, mac_key.end());
mac_input.insert(mac_input.end(), encrp.ciphertext.begin(), encrp.ciphertext.end());
Keccak256::getHash(mac_input.data(), mac_input.size(), mac.data());
...
bytes_data decrypted_masternode = crypto_utils::AES_256_CTR(encryption_key, encrp.ciphertext, encrp.iv);
