...
void AsyncTransactionsHistoryManager::request_transactions_data(const std::string& eth_addr) {
    if (updating)
      return;
    ...
    worker = std::async(std::launch::async, [eth_addr]() {
      return BlockchainClient::get_transaction_history(eth_addr);
    });
}
...
std::vector<TransactionRecord> AsyncTransactionsHistoryManager::get_transactions_history(void) const {
