...
void RpcBridge::start(int port) {
  ...
  server.Post("/", [this](const httplib::Request &req, httplib::Response &res) {
    try {
      ...
      json error;
      error["jsonrpc"] = "2.0";
      error["error"] = {{"code", -32700}, {"message", err.what()}};
  ...
  running = true;
  server_th = std::thread([this, port] { server.listen("127.0.0.1", port); });
}
...
std::shared_ptr<DappRequest>
RpcBridge::submit_and_wait(DappRequestType type, const json &params,
...
json RpcBridge::handle_rpc(const json &body, const std::string &origin) {
  ...
  if (method == "eth_chainId") {
    ...
    auto req =
        submit_and_wait(DappRequestType::RequestAccounts, params, origin);
    if (!req->error.empty()) {
      res["error"] = {{"code", 4401}, {"message", req->error}};
      return res;
    }
    ...
    auto req =
        submit_and_wait(DappRequestType::SendTransaction, params, origin);
    if (!req->error.empty()) {
      res["error"] = {{"code", 4001}, {"message", req->error}};
      return res;
    }
    ...
    auto type = method == "personal_sign" ? DappRequestType::PersonalSign
...
bool RpcBridge::is_running(void) const { return running; }
