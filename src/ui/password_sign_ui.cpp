...
Component PasswordUI::create(const std::string &title,
                             std::function<bool(const secure_string &)> verify,
  ...
  auto form = Renderer(input, [=] {
    return vbox({vbox({text(" 🔐 " + title + " ") | bold | color(Color::Cyan) |
                           hcenter,
                       ...
                       error_msg->empty() ? emptyElement()
                                          : text_(*error_msg) | hcenter | bold |
                                                color(Color::Red),

                       separator(),
                       text(" [ENTER] Confirm | [ESC] Cancel ") | dim |
                           hcenter}) |
                 borderHeavy | color(Color::Yellow) | size(WIDTH, EQUAL, 50) |
