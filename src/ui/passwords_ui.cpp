...
Component CLI::render_password_setup(void) {
  ...
  return Renderer(first_stage, [=, this] {
    ...
    auto content =
        vbox({header, separatorDouble(), filler(), warning_box, filler(),
              text(" INPUT IS HIDDEN FOR YOUR SECURITY ") | hcenter | dim,
              input_box});

    return to_center(content | borderHeavy | size(WIDTH, EQUAL, 70) |
...
Component CLI::render_confirm_password_setup(void) {
  ...
  return Renderer(second_stage, [=, this] {
    ...
    if (!*is_incorrect) {
      info_lines = {text("Please re-enter your password to verify."),
    ...
    auto content =
        vbox({header, separatorDouble(), filler(),
              vbox(std::move(info_lines)) | hcenter, filler(), input_box,
              filler(), separator(),
...
Component CLI::render_request_unlock_password(void) {
  ...
  return Renderer(component, [=, this] {
    ...
    return to_center(content | borderHeavy | size(WIDTH, EQUAL, 60) |
